...
| Dock pose set gates (charging / RTK σ / yaw convergence / GPS averaging / yaw_source) | `area_manager.cpp` `on_set_docking_point`; write-back via `mowgli_interfaces/robot_yaml_scalar.hpp` `UpdateDockPose`. The `is_charging` gate is skipped for exactly one combination — `yaw_source=MOTION && !use_gps_position` (mowglinext#446) — the one-click dock calibration's pre-reverse capture, which fires after the robot has deliberately left the dock; every other combination keeps the gate |
