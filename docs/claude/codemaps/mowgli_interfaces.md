...
- **`GnssStatus.msg` enums / `CAP_*` bits** ↔ the mapping tables in `sensors/gps/universal_gnss_topic_bridge.py:19-24` (and `sensors/gps/mowgli_gnss_bridge/`) that translate `universal_gnss_msgs/GnssStatus`; `sensors-gps.yml` rebuilds on any change here.
...
- `ros2/src/external/universal-gnss/universal_gnss_msgs/` — vendored interface-only package defining the upstream `universal_gnss_msgs/GnssStatus` bridged into Mowgli's public status.
