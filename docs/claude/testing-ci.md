...
| `sensors-gps.yml` | push (same branch set), paths `sensors/gps/**`, `sensors/README.md`, `ros2/src/mowgli_interfaces/**`, `ros2/src/external/universal-gnss/**`, self, `_sensor-docker.yml` | via `_sensor-docker.yml` | Builds the `gps` image; the only sensor caller with a real smoke test (L37–66): asserts `mowgli_interfaces`, `universal_gnss_msgs` and `mowgli_gnss_bridge` are importable, checks that `universal_gnss_msgs` has no executables, and validates `GnssStatus.CAP_RTK_MODE`, `RtcmFrame.data` and `rtcm_msgs/Message.message`. The full Universal GNSS runtime remains in the `mowgli-gps` sidecar and is not built into `mowgli-ros2`. | — | — |
