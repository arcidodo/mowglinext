...
| `universal_gnss_msgs` — vendored submodule | `ros2/src/external/universal-gnss/universal_gnss_msgs/tests/test_interface_only.py` | pytest/static package-boundary check | Tested upstream; Mowgli CI builds it as the bridge's interface-only dependency | UG CI + `ros2-ci.yml` |
...
Two upstream test piles deliberately do not execute in MowgliNext: Universal GNSS runtime tests (`gnss_core/`, `gnss_protocols/`, `gnss_driver/`, `gnss_transport/`, `gnss_ntrip/`, `gnss_tools/`, `gnss_ros2/`) belong to the external sidecar repository, while this repo builds/tests only `universal_gnss_msgs` plus `mowgli_gnss_bridge`; and the upstream `ros2/src/opennav_coverage/*/test/*.cpp` suites are `COLCON_IGNORE`'d because only `opennav_coverage_msgs` is built.
...
| `ros2-docker.yml` | push (same branch set) + tags `v*.*.*`, paths `ros2/**`, `sensors/gps/mowgli_gnss_bridge/**`, `tools/motor/**`, self; `workflow_dispatch` (`no-cache`) | `build` (amd64 + arm64 native runners), `merge`, `notify-failure` | Builds `ghcr.io/<repo>/mowgli-ros2` `runtime` target, then checks `mowgli_gnss_bridge` + `universal_gnss_msgs`, zero executables in the message package, and absence of `universal_gnss_ros2` plus receiver/NTRIP/replay nodes | — | `sbom: false` — syft's SPDX doc exceeds buildkit's hardcoded 40 MiB attestation cap; `provenance` stays on. `notify-failure` opens a `ci-failure` issue only on `main`. |
...
| `sensors-gps.yml` | push (same branch set), paths `sensors/gps/**`, `sensors/README.md`, `ros2/src/mowgli_interfaces/**`, `ros2/src/external/universal-gnss/**`, self; `workflow_dispatch` | `test` | Assembles exactly `mowgli_interfaces`, `universal_gnss_msgs`, `mowgli_gnss_bridge`; builds the two interface packages, then builds/tests the bridge and requires non-zero gtest cases. Publishes no GPS image. The full Universal GNSS runtime is the external `mowgli-gps` sidecar (`v0.1.4-rc1`). | — | — |
...
make build-dev           # focused set: mowgli_interfaces mowgli_localization universal_gnss_msgs mowgli_bringup
