...
recompute_image_defaults() {
  ...
  UNIVERSAL_GNSS_IMAGE_DEFAULT="ghcr.io/pepeuch/universal-gnss-ros2-kilted:v0.1.3-rc1@sha256:781579c2cf79a6932ae8d64e8b395957de09f59f638df1136086351cd66e7e40"
}
...
write_mavros_runtime_config() {
  ...
  mkdir -p "$(dirname "$target")"
  cp "$source" "$target"
  # The external MAVROS image understands this existing key. Its receiver and
...
EOF
  ...
  # Universal GNSS owns its receiver and NTRIP parameters in a separate
  # image-specific file. The public Mowgli YAML remains the operator-facing
  # source of truth; the adapter reads DDS only and owns no hardware.
  write_universal_gnss_parameters \
